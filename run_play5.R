...
# Load necessary library for multinomial regression
library(nnet)
...
decision_data <- data.frame(
  FP = sample(10:80, 300, replace = TRUE),   # Field position
  YTG = sample(1:10, 300, replace = TRUE),   # Yards to go
  Decision = sample(c("go for it", "punt", "field goal"), 300, replace = TRUE, 
                    prob = c(0.3, 0.5, 0.2)) # Randomly assigned decisions
)
